...
If git command cannot be installed, download zip. Once iris-web-master.zip is downloaded, unzip it to the /opt directory 

```python
sudo unzip iris-web-master.zip -d /opt
```
...
After pulling the images, save them as `.tar` files:
...
Copy the `.tar` files to the air-gapped system and load them:
